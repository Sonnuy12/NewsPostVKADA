...
final class NavigationBarManager {
    ...
    static func configureNavigationBar(for viewController: UIViewController, withAction action: Selector) {
        ...
        // Загрузка данных пользователя из UserDefaults
        let userDefaults = UserDefaults.standard
        let firstName = userDefaults.string(forKey: "UserFirstName") ?? "Гость"
        let lastName = userDefaults.string(forKey: "UserLastName") ?? ""
        nameLabel.text = firstName.isEmpty ? "Гость" : "\(firstName) \(lastName)"
        
        if let avatarURLString = userDefaults.string(forKey: "UserAvatarURL"),
